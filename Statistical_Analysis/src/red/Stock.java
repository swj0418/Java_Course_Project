...
public class Stock {
	...
	public void retrieve() {
		System.out.println("Retrieving " + this.SYMBOL);
		checkLocal(); //First, check if the data is there and is up-to-date.

		if(localAvailability != true) {
			System.out.println(this.SYMBOL + " is not locally available");
			URL url;
			...
			*/
			String urlString = "https://www.alphavantage.co/query?function=TIME_SERIES_DAILY_ADJUSTED&symbol=" +
					SYMBOL.trim() +"&apikey=QX75TG29OOA9H7CJ&datatype=csv&outputsize=full";
...
//		 * Should I reverse?
		 */
		FileWriter fw;
		String path = "./Data/Historical/" + SYMBOL.trim(); // trim is crucial for that a user might accidentally add space
